...
class BaseTest(TestCase):
    def setUp(self) -> None:
        ...
        self.choice_2 = Choice.objects.create(
            question=self.question, choice_text="Test Choice 2"
        )
...
class PrivatePollTest(BaseTest):
    def test_retrieve_poll_detail(self) -> None:
        poll_detail_url = reverse(
            "polls:poll-detail", kwargs={"slug": self.poll.slug}
        )
    ...
    def test_submit_poll(self) -> None:
        ...
        response = self.client.post(
            poll_detail_url, {f"choice_{self.choice_1.id}": self.choice_1.id}
        )

        self.assertEqual(response.status_code, 302)
        self.assertEqual(
            response.url, reverse("polls:poll-results", args=(self.poll.slug,))
        )

        self.assertTrue(
            Answer.objects.filter(
                owner=self.user, choice=self.choice_1
            ).exists()
        )
    ...
    def test_create_poll(self) -> None:
        ...
        self.assertRedirects(
            response, reverse("polls:poll-detail", kwargs={"slug": poll.slug})
        )

    def test_poll_delete(self) -> None:
        response = self.client.post(
            reverse("polls:poll-delete", kwargs={"slug": self.poll.slug})
        )
...
class PublicPollTest(BaseTest):
    ...
    def test_retrieve_poll_results(self) -> None:
        poll_results_url = reverse(
            "polls:poll-results", kwargs={"slug": self.poll.slug}
        )
